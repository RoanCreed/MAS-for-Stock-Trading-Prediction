...
namespace MAS_Coursework_Double_Auction
{
    public class CoordinatorAgent : Agent
    {
        ...
        private void CalculateResult()
        {
            ...
            {   //Buy signal
                ...
                if (d[dateIndex + 1].close > d[dateIndex].close)
                {
                    ...
                    TAAList[maxIndex].successfulTrades = TAAList[maxIndex].successfulTrades + 1;
                ...
                else
                {
                    ...
                    TAAList[maxIndex].reputation = TAAList[maxIndex].reputation - 0.2;
                    TAAList[maxIndex].unsuccessfulTrades = TAAList[maxIndex].unsuccessfulTrades + 1;
                }
            }
        ...
        public void HandleFinish()
        {
            ...
            foreach(var agent in TAAList)
            {
                double agentSuccessRate = 0;
                if ((agent.successfulTrades + agent.unsuccessfulTrades) > 0)
                {
                    agentSuccessRate = agent.successfulTrades / (agent.successfulTrades + agent.unsuccessfulTrades);
                }
                

                Console.WriteLine($"{agent.agentName} <><><> Reputation: {agent.reputation} <><><> Success accuracy: {agentSuccessRate}");
            }
