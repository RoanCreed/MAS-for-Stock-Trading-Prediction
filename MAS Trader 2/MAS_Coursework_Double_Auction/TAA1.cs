    ...
    public class TAA1 : Agent
    {
        ...
        public override void Act(Message message)   //Program enters here after it receives a message
        {
            try
            {
                ...
                {
                    case "start":
                        Console.WriteLine($"{Name} - Initialising SMA calculation for: " + parameters[0]);
        ...
        private (double, int) HandleSMA(string stock, DateTime startDate)
        {
            ...
            if (SMA < d[index].close)    //Current day stock price
            {
                ...
                signal = 1;
                probability = 1;
                Console.WriteLine($"{Name} - Stock: {stock} is in an uptrend - Current price: {d[index].close} - SMA: {SMA}");
            ...
            else if (SMA > d[index].close)
            {
                ...
                signal = 0;
                probability = 1;
                Console.WriteLine($"{Name} - Stock: {stock} is in a downtrend - Current price: {d[index].close} - SMA: {SMA}");
            ...
            else
            {
                ...
                signal = 0;
                probability = 0;
                Console.WriteLine($"{Name} - Stock: {stock} has no change - Current price: {d[index].close} - SMA: {SMA}");
